...
SELECT * FROM 주문;
SELECT * FROM 배송업체;
SELECT 주문고객 FROM 주문; // 주문 테이블에서 주문고객 검색
SELECT 제품명, 단가 AS 가격 
FROM 제품; // 단가가 가격으로 바뀌어서 출력

ALTER TABLE 고객 ADD 가입날짜 DATE;
ALTER TABLE 고객 DROP COLUMN 가입날짜;
ALTER TABLE 고객 ADD CONSTRAINT CHK_AGE CHECK (나이 >= 20); // 제약조건창에서 확인
...
INSERT INTO 고객 VALUES ('peach', '오형준', '', 'silver', '의사', 300);
